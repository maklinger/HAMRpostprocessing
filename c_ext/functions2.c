...
void kernel_rdump_new(int flag, int RAD_M1, int RESISTIVE, int DOTRACER,int TWO_T, int P_NUM,int DO_YE, int DO_NUCLEAR, int NEUTRINOS_M1, char *dir, int dump, int n_active_total, int f1,int f2,int f3, int nb, int bs1, int bs2, int bs3, float* rho,float* index_tr1,float* index_tr2,float* ug, float* uu, float* B,float* E,float* E_rad,float* uu_rad,float* TE,float* TI,float* photon_number,float* Ye,float* X_alpha,float* X_atm, float* E_nu1,float* E_nu2,float* E_nu3,float* uu_nu1,float* uu_nu2,float* uu_nu3,float* nu_number1, float* nu_number2, float* nu_number3,float* gcov,float* gcon, int axisym);
...
void kernel_rdump_griddata(int flag, int interpolate,int RAD_M1,int RESISTIVE, int DOTRACER,int TWO_T, int P_NUM, int DO_YE, int DO_NUCLEAR, int NEUTRINOS_M1, char *dir, int dump, int n_active_total, int f1,int f2,int f3, int nb, int bs1, int bs2, int bs3, float* rho,float* index_tr1,float* index_tr2,float* ug, float* uu, float* B,float* E,float* E_rad,float* uu_rad,float* TE,float* TI,float* photon_number,float* Ye,float* X_alpha,float* X_atm, float* E_nu1,float* E_nu2,float* E_nu3,float* uu_nu1,float* uu_nu2,float* uu_nu3,float* nu_number1, float* nu_number2, float* nu_number3,float* gcov,float* gcon,
...
void kernel_rdump_new(int flag, int RAD_M1, int RESISTIVE, int DOTRACER, int TWO_T, int P_NUM, int DO_YE, int DO_NUCLEAR, int NEUTRINOS_M1, char *directory, int dump, int n_active_total, int f1,int f2,int f3, int nb, int bs1, int bs2, int bs3, float* rho, float* index_tr1, float* index_tr2,float* ug, float* uu, float* B,float* E,float* E_rad,float* uu_rad,float* TE,float* TI,float* photon_number,float* Ye,float* X_alpha,float* X_atm, float* E_nu1,float* E_nu2,float* E_nu3,float* uu_nu1,float* uu_nu2,float* uu_nu3,float* nu_number1, float* nu_number2, float* nu_number3,float* gcov,float* gcon, int axisym){
    #pragma omp parallel
	{
        ...
        int NPR=9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+18*NEUTRINOS_M1+2*DOTRACER;
        char filename1[100],filename2[100];
        float trash[14];
        FILE *fin;
                    ...
                    for(n=n_start; n<n_end; n++){
                        if(u%num_threads==thread_id){
                            for(i=0;i<bs1new;i++)for(j=0;j<bs2new;j++)for(z=0;z<bs3new;z++){
                                ...
                                if(DO_YE==1){
                                    Ye[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM];
                                }
                                if(DO_NUCLEAR==1){
                                    X_alpha[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE];
                                    X_atm[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+1];
                                }
                                if(NEUTRINOS_M1==1){
                                    E_nu1[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR];
                                    for(ii=0;ii<4;ii++) uu_nu1[(ii)*gridsize_3D+index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 1];
                                    nu_number1[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+5];

                                    E_nu2[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+6];
                                    for(ii=0;ii<4;ii++) uu_nu2[(ii)*gridsize_3D+index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 7];
                                    nu_number2[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+11];

                                    E_nu3[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+12];
                                    for(ii=0;ii<4;ii++) uu_nu3[(ii)*gridsize_3D+index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 13];
                                    nu_number3[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+17];
                                }
                                if(DOTRACER==1){
                                    index_tr1[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+18*NEUTRINOS_M1];
                                    index_tr2[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+18*NEUTRINOS_M1+1];
                                }
                            }
                        }
...
void kernel_rdump_griddata(int flag, int interpolate, int RAD_M1, int RESISTIVE, int DOTRACER, int TWO_T, int P_NUM, int DO_YE, int DO_NUCLEAR, int NEUTRINOS_M1, char *directory, int dump, int n_active_total, int f1,int f2,int f3, int nb, int bs1, int bs2, int bs3, float* rho, float* index_tr1, float* index_tr2,float* ug, float* uu, float* B,float* E,float* E_rad,float* uu_rad,
float* TE,float* TI,float* photon_number,float* Ye,float* X_alpha,float* X_atm, float* E_nu1,float* E_nu2,float* E_nu3,float* uu_nu1,float* uu_nu2,float* uu_nu3,float* nu_number1, float* nu_number2, float* nu_number3,float* gcov,float* gcon,
...
float a, float gam, float* Rdot, float* bsq, float* r, float startx1, float startx2, float startx3, float _dx1, float _dx2, float _dx3, float* x1, float* x2, float* x3, int i_min, int i_max, int j_min, int j_max, int z_min, int z_max){
    float *rho_grad,*index_tr1_grad,*index_tr2_grad, *ug_grad, *uu_grad, *B_grad,*E_grad, *E_rad_grad, *uu_rad_grad, *Rdot_grad, *TE_grad, *TI_grad, *photon_number_grad;
    float *Ye_grad, *X_alpha_grad, *X_atm_grad, *E_nu1_grad, *E_nu2_grad, *E_nu3_grad, *uu_nu1_grad, *uu_nu2_grad, *uu_nu3_grad, *nu_number1_grad, *nu_number2_grad, *nu_number3_grad;
    int float_size = sizeof(float);
    int gridsize_3D;
    ...
    if(interpolate){
        ...
        if(DO_YE){
            Ye_grad=(float *)malloc(gridsize_3D*sizeof(float));
        }
        if(DO_NUCLEAR){
            X_alpha_grad=(float *)malloc(gridsize_3D*sizeof(float));
            X_atm_grad=(float *)malloc(gridsize_3D*sizeof(float));
        }
        if(NEUTRINOS_M1){
            E_nu1_grad=(float *)malloc(gridsize_3D*sizeof(float));
            E_nu2_grad=(float *)malloc(gridsize_3D*sizeof(float));
            E_nu3_grad=(float *)malloc(gridsize_3D*sizeof(float));
            uu_nu1_grad=(float *)malloc(4*gridsize_3D*sizeof(float));
            uu_nu2_grad=(float *)malloc(4*gridsize_3D*sizeof(float));
            uu_nu3_grad=(float *)malloc(4*gridsize_3D*sizeof(float));
            nu_number1_grad=(float *)malloc(gridsize_3D*sizeof(float));
            nu_number2_grad=(float *)malloc(gridsize_3D*sizeof(float));
            nu_number3_grad=(float *)malloc(gridsize_3D*sizeof(float));
        }
        if(RAD_M1){
        ...
        if(DOTRACER){
            index_tr1_grad=(float *)malloc(gridsize_3D*sizeof(float));
            index_tr2_grad=(float *)malloc(gridsize_3D*sizeof(float));
        }
    }
                            ...
                            for(i2=(coord1)*((int)pow(1+REF_1,P1)*bs1)+offset1; i2<(coord1+1)*((int)pow(1+REF_1,P1)*bs1); i2+=(1*export_raytracing+f1*(1-export_raytracing)))for(j2=(coord2)*((int)pow(1+REF_2,P2)*bs2)+offset2; j2<(coord2+1)*((int)pow(1+REF_2,P2)*bs2); j2+=(1*export_raytracing+f2*(1-export_raytracing)))for(z2=(coord3)*((int)pow(1+REF_3,P3)*bs3)+offset3; z2<(coord3+1)*((int)pow(1+REF_3,P3)*bs3); z2+=(1*export_raytracing+f3*(1-export_raytracing))){
                                ...
                                if(i2/f1>=i_min && i2/f1<i_max && j2/f2>=j_min && j2/f2<j_max && z2/f3>=z_min && z2/f3<z_max){
                                    ...
                                    if(export_raytracing==1){
                                        ...
                                        if(DO_YE){
                                            Ye[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM]/volume;
                                        }
                                        if(DO_NUCLEAR){
                                            X_alpha[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE]/volume;
                                            X_atm[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+1]/volume;
                                        }
                                        if(NEUTRINOS_M1){
                                            E_nu1[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR]/volume;
                                            for(ii=1;ii<4;ii++)vkerr[ii]=0.;
                                            gamma=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 1]*alpha;
                                            for(ii=0;ii<4;ii++) uu_nu1[(ii)*gridsize_3D+index_3D]+=(gamma*beta[ii]/alpha + trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 1 + ii])/volume;
                                            nu_number1[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+5]/volume;

                                            E_nu2[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+6]/volume;
                                            for(ii=1;ii<4;ii++)vkerr[ii]=0.;
                                            gamma=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 7]*alpha;
                                            for(ii=0;ii<4;ii++) uu_nu2[(ii)*gridsize_3D+index_3D]+=(gamma*beta[ii]/alpha + trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 7 + ii])/volume;
                                            nu_number2[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+11]/volume;

                                            E_nu3[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+12]/volume;
                                            for(ii=1;ii<4;ii++)vkerr[ii]=0.;
                                            gamma=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 13]*alpha;
                                            for(ii=0;ii<4;ii++) uu_nu3[(ii)*gridsize_3D+index_3D]+=(gamma*beta[ii]/alpha + trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 13 + ii])/volume;
                                            nu_number3[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+17]/volume;
                                        }
                                        if(DOTRACER){
                                            index_tr1[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+18*NEUTRINOS_M1]/volume;
                                            index_tr2[index_3D]+=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+18*NEUTRINOS_M1+1]/volume;
                                        }
                                    }
                                    else{
                                        rho[index_3D]=trash[0];
                                       ...
                                       if(interpolate){
                                        ...
                                        if(NEUTRINOS_M1==1){
                                            ...
                                            if(interpolate){
                                                for(ii=1;ii<4;ii++)vkerr[ii]=0.;
                                                gamma=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 1]*alpha;
                                                for(ii=0;ii<4;ii++) uu_nu1[(ii)*gridsize_3D+index_3D]=(gamma*beta[ii]/alpha + trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 1 + ii]);
                                            }
                                            else{
                                                for(ii=0;ii<4;ii++) uu_nu1[(ii)*gridsize_3D+index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 1 + ii];
                                            }
                                            nu_number1[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+5];
                                            ...
                                            if(interpolate){
                                                for(ii=1;ii<4;ii++)vkerr[ii]=0.;
                                                gamma=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 7]*alpha;
                                                for(ii=0;ii<4;ii++) uu_nu2[(ii)*gridsize_3D+index_3D]=(gamma*beta[ii]/alpha + trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 7 + ii]);
                                            }
                                            else{
                                                for(ii=0;ii<4;ii++) uu_nu2[(ii)*gridsize_3D+index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 7 + ii];
                                            }
                                            nu_number2[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+11];
                                            ...
                                            if(interpolate){
                                                for(ii=1;ii<4;ii++)vkerr[ii]=0.;
                                                gamma=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 13]*alpha;
                                                for(ii=0;ii<4;ii++) uu_nu3[(ii)*gridsize_3D+index_3D]=(gamma*beta[ii]/alpha + trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 13 + ii]);
                                            }
                                            else{
                                                for(ii=0;ii<4;ii++) uu_nu3[(ii)*gridsize_3D+index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 13 + ii];
                                            }
                                            nu_number3[index_3D]=trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+17];
                                        }
                                        if(DOTRACER){
							...
							for(i2=(coord1)*((int)pow(1+REF_1,P1)*bs1)+offset1; i2<(coord1+1)*((int)pow(1+REF_1,P1)*bs1); i2+=f1)for(j2=(coord2)*((int)pow(1+REF_2,P2)*bs2)+offset2; j2<(coord2+1)*((int)pow(1+REF_2,P2)*bs2); j2+=f2)for(z2=(coord3)*((int)pow(1+REF_3,P3)*bs3)+offset3; z2<(coord3+1)*((int)pow(1+REF_3,P3)*bs3); z2+=f3){
								...
								if(i2>=i_min && i2<i_max && j2>=j_min && j2<j_max && z2>=z_min && z2<z_max){
                                    ...
                                    if(fabs(x_left-x_middle)>0.000001 && fabs(x_right-x_middle)>0.000001){
                                        ...
                                        if(DOTRACER){
                                            index_tr1_grad[index_3D]=0.0;
                                            index_tr2_grad[index_3D]=0.0;
                                            if(fabs(x_left-x_middle)>0.000001 && fabs(x_right-x_middle)>0.000001){
                                                index_tr1_grad[index_3D]=slope_lim(index_tr1[left],index_tr1[middle],index_tr1[right], x_left, x_middle, x_right);
                                                index_tr2_grad[index_3D]=slope_lim(index_tr2[left],index_tr2[middle],index_tr2[right], x_left, x_middle, x_right);
                                                }
                                        }
									}
								}
                       ...
                       if(n%num_threads==thread_id){
                            ...
                            for(i2=(coord1)*((int)pow(1+REF_1,P1)*bs1)+offset1; i2<(coord1+1)*((int)pow(1+REF_1,P1)*bs1); i2+=f1)for(j2=(coord2)*((int)pow(1+REF_2,P2)*bs2)+offset2; j2<(coord2+1)*((int)pow(1+REF_2,P2)*bs2); j2+=f2)for(z2=(coord3)*((int)pow(1+REF_3,P3)*bs3)+offset3; z2<(coord3+1)*((int)pow(1+REF_3,P3)*bs3); z2+=f3){
                                ...
                                if(i2>=i_min && i2<i_max && j2>=j_min && j2<j_max && z2>=z_min && z2<z_max){
                                    ...
                                    if(DO_YE){
                                        Ye[index_3D]=Ye[index_3D]+offset*Ye_grad[index_3D];
                                    }

                                    if(DO_NUCLEAR){
                                        X_alpha[index_3D]=X_alpha[index_3D]+offset*X_alpha_grad[index_3D];
                                        X_atm[index_3D]=X_atm[index_3D]+offset*X_atm_grad[index_3D];
                                    }

                                    if(NEUTRINOS_M1){
                                        E_nu1[index_3D]= E_nu1[index_3D]+offset*E_nu1_grad[index_3D];
                                        for(ii=0; ii<4; ii++)uu_nu1[ii*gridsize_3D+index_3D]=uu_nu1[ii*gridsize_3D+index_3D]+offset*uu_nu1_grad[ii*gridsize_3D+index_3D];

                                        E_nu2[index_3D]= E_nu2[index_3D]+offset*E_nu2_grad[index_3D];
                                        for(ii=0; ii<4; ii++)uu_nu2[ii*gridsize_3D+index_3D]=uu_nu2[ii*gridsize_3D+index_3D]+offset*uu_nu2_grad[ii*gridsize_3D+index_3D];

                                        E_nu3[index_3D]= E_nu3[index_3D]+offset*E_nu3_grad[index_3D];
                                        for(ii=0; ii<4; ii++)uu_nu3[ii*gridsize_3D+index_3D]=uu_nu3[ii*gridsize_3D+index_3D]+offset*uu_nu3_grad[ii*gridsize_3D+index_3D];
                                    }
                                    if(DOTRACER){
                                        index_tr1[index_3D]=index_tr1[index_3D]+offset*index_tr1_grad[index_3D];
                                        index_tr2[index_3D]=index_tr2[index_3D]+offset*index_tr2_grad[index_3D];
                                    }
                                }
                                i2*=f1;
                                j2*=f2;
                ...
                if(n%num_threads==thread_id){
                    ...
                    for(i2=(coord1)*((int)pow(1+REF_1,P1)*bs1)+offset1; i2<(coord1+1)*((int)pow(1+REF_1,P1)*bs1); i2+=f1)for(j2=(coord2)*((int)pow(1+REF_2,P2)*bs2)+offset2; j2<(coord2+1)*((int)pow(1+REF_2,P2)*bs2); j2+=f2)for(z2=(coord3)*((int)pow(1+REF_3,P3)*bs3)+offset3; z2<(coord3+1)*((int)pow(1+REF_3,P3)*bs3); z2+=f3){
                        ...
                        if(i2>=i_min && i2<i_max && j2>=j_min && j2<j_max && z2>=z_min && z2<z_max){
                            ...
                            //Neutrino radiation variables
                            if(NEUTRINOS_M1){
                              //Do velocity conversion
                                for(ii=0; ii<4; ii++)vkerr[ii]=uu_nu1[ii*gridsize_3D+index_3D];
                                gamma=gcov[(1*4+1)*gridsize_2D+index_2D]*vkerr[1]*vkerr[1] + gcov[(2*4+2)*gridsize_2D+index_2D]*vkerr[2]*vkerr[2]+ gcov[(3*4+3)*gridsize_2D+index_2D]*vkerr[3]*vkerr[3]
                                   + 2.*(gcov[(1*4+2)*gridsize_2D+index_2D]*vkerr[1]*vkerr[2] + gcov[(1*4+3)*gridsize_2D+index_2D]*vkerr[1]*vkerr[3] + gcov[(2*4+3)*gridsize_2D+index_2D]*vkerr[2]*vkerr[3]);
                                gamma=sqrt(1.+gamma);
                                uu_nu1[index_3D]= gamma/alpha;
                                for(ii=1;ii<4;ii++){
                                    uu_nu1[(ii)*gridsize_3D+index_3D]= vkerr[ii] - gamma*beta[ii]/alpha ;
                                }

                                for(ii=0; ii<4; ii++)vkerr[ii]=uu_nu2[ii*gridsize_3D+index_3D];
                                gamma=gcov[(1*4+1)*gridsize_2D+index_2D]*vkerr[1]*vkerr[1] + gcov[(2*4+2)*gridsize_2D+index_2D]*vkerr[2]*vkerr[2]+ gcov[(3*4+3)*gridsize_2D+index_2D]*vkerr[3]*vkerr[3]
                                   + 2.*(gcov[(1*4+2)*gridsize_2D+index_2D]*vkerr[1]*vkerr[2] + gcov[(1*4+3)*gridsize_2D+index_2D]*vkerr[1]*vkerr[3] + gcov[(2*4+3)*gridsize_2D+index_2D]*vkerr[2]*vkerr[3]);
                                gamma=sqrt(1.+gamma);
                                uu_nu2[index_3D]= gamma/alpha;
                                for(ii=1;ii<4;ii++){
                                    uu_nu2[(ii)*gridsize_3D+index_3D]= vkerr[ii] - gamma*beta[ii]/alpha ;
                                }

                                for(ii=0; ii<4; ii++)vkerr[ii]=uu_nu3[ii*gridsize_3D+index_3D];
                                gamma=gcov[(1*4+1)*gridsize_2D+index_2D]*vkerr[1]*vkerr[1] + gcov[(2*4+2)*gridsize_2D+index_2D]*vkerr[2]*vkerr[2]+ gcov[(3*4+3)*gridsize_2D+index_2D]*vkerr[3]*vkerr[3]
                                   + 2.*(gcov[(1*4+2)*gridsize_2D+index_2D]*vkerr[1]*vkerr[2] + gcov[(1*4+3)*gridsize_2D+index_2D]*vkerr[1]*vkerr[3] + gcov[(2*4+3)*gridsize_2D+index_2D]*vkerr[2]*vkerr[3]);
                                gamma=sqrt(1.+gamma);
                                uu_nu3[index_3D]= gamma/alpha;
                                for(ii=1;ii<4;ii++){
                                    uu_nu3[(ii)*gridsize_3D+index_3D]= vkerr[ii] - gamma*beta[ii]/alpha ;
                                }
                            }
                        }
                        i2*=f1;
    ...
    if(interpolate){
        ...
        if(NEUTRINOS_M1){
            ...
            free(uu_nu2_grad);
            free(uu_nu3_grad);
            free(nu_number1_grad);
            free(nu_number2_grad);
            free(nu_number3_grad);
        }
        if(DOTRACER){
            free(index_tr1_grad);
            free(index_tr2_grad);
    ...
    for(u=0;u<umax;u++){
        ...
        for(n=n_start; n<n_end; n++){
            for(i=0;i<bs1new;i++)for(j=0;j<bs2new;j++)for(z=0;z<bs3new;z++){
                ...
                if(DO_YE==1){
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM]=Ye[index_3D];
                }
                if(DO_NUCLEAR==1){
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE]=X_alpha[index_3D];
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+1]=X_atm[index_3D];
                }
                if(NEUTRINOS_M1==1){
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR]=E_nu1[index_3D];
                    for(ii=0;ii<4;ii++) trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 1 + ii]=uu_nu1[(ii)*gridsize_3D+index_3D];
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+5]=nu_number1[index_3D];

                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+6]=E_nu2[index_3D];
                    for(ii=0;ii<4;ii++) trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 7 + ii]=uu_nu2[(ii)*gridsize_3D+index_3D];
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+11]=nu_number2[index_3D];

                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+12]=E_nu3[index_3D];
                    for(ii=0;ii<4;ii++) trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR + 13 + ii]=uu_nu3[(ii)*gridsize_3D+index_3D];
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+17]=nu_number3[index_3D];
                }
                if(DOTRACER==1){
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+18*NEUTRINOS_M1]=index_tr1[index_3D];
                    trash[9+5*RAD_M1+3*RESISTIVE+2*TWO_T+1*P_NUM+1*DO_YE+2*DO_NUCLEAR+18*NEUTRINOS_M1+1]=index_tr2[index_3D];
                }
